...
- String safety refactor log (cont.): tightened `src/act_obj.c` coin handling (pickups, sacrifices, and large drop logs) and combat remove logging with `snprintf` to keep small buffers bounded; continued sweep needed for the many remaining legacy string operations throughout `src/act_obj.c` and other modules.
