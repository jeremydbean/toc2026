...
- **Web admin service**: `webadmin/server.py` exposes a FastAPI app with HTML UI plus JSON endpoints to append actions into `/app/area/webadmin.queue` (wizinfo broadcast, immortal command, backup, shutdown). It also provides parsed area browsing, area-health results from `webadmin/area_health.py`, backup archive listing, token-protected log tailing, and an area reload endpoint for dashboard data. Health checks inspect `merc` and `uvicorn` process/port state; log tailing reads the configured `LOG_FILE`.
...
## Current Validation & Operations (July 2026)
- **Full validation, Windows**: run `.\scripts\validate.ps1` from PowerShell. The script uses WSL for Linux C builds and `merc --check-area`, then runs Python syntax checks, area reference checks, area-health linting, unit tests, and `git diff --check` from the Windows checkout. Use `-RunSmoke` for a short live startup smoke test and `-Distro <name>` when the WSL distro is not `Ubuntu`.
- **Full validation, Linux/macOS/CI**: run `bash scripts/validate.sh`. GitHub Actions uses this script through `.github/workflows/validate.yml`.
- **Focused area validation**: from `area/`, run `../merc --check-area` (or `../merc --validate`) to boot the world database and exit without opening a listening socket.
- **Area-health linter**: run `python scripts/area_lint.py --fail-on critical --limit 20`. It currently treats parse failures, missing listed area files, duplicate vnums, and exits to positive missing room vnums as critical. Warnings and info findings are cleanup targets unless the caller asks for a stricter threshold.
- **Current area-health baseline**: the repository should have zero critical issues. The known warning/info backlog is mostly disconnected room groups, unusual level values, one-way exits, unspawned mobs, and objects without reset sources.
- **Operations commands**: `diagnostics` is an L1/God command that prints boot time, world totals, descriptor count, list sizes, active mobs, and backup schedule. `backup` remains the in-game backup command and can trigger pfile or daily backups.
- **Backup model**: scheduled/admin backups create `backups/*.tar.gz` archives from `player/` and prune archives older than 30 days. Player saves also maintain per-player snapshots in `player/versions/<Name>/`, throttled by `PLAYER_SNAPSHOT_MIN_INTERVAL` and capped by `PLAYER_VER_MAX`.
- **More detail**: see `wiki/validation-and-area-health.md` for validation commands, issue codes, CI behavior, and troubleshooting.
...
- **String safety module**: `src/string_safe.c` provides bounded `toc_strlcpy()` and `toc_strlcat()` helpers with proper null-termination and overflow protection.
...
- **Web admin**: FastAPI dashboard in `webadmin/server.py`, area parsing in `webadmin/area_parser.py`, shared linting in `webadmin/area_health.py`, and tests under `tests/`.
- **Area data**: `area/` contains 100+ `.are` files (rooms, mobs, objects, resets). The old ident helper now lives at `archive/resolve.c`.
...
### Historical unsafe patterns found
1. **`src/act_info.c` lines 116, 336**: Two `strcat()` calls without bounds checking:
   ...
   - **Status**: Fixed by replacing the unbounded concatenation with bounded string handling.

2. **`archive/resolve.c` lines 80, 86, 131**: Legacy ident resolver with three unsafe calls:
   - Line 80: `strcpy(addr_str, ...)`  
   - Line 86: `sprintf(addr_str, "%d.%d.%d.%d", ...)`  
   - Line 131: `sprintf(request, "%d,%d", ...)`
   - **Impact**: Archived module is not compiled or linked, so it does not affect runtime safety.
   - **Status**: Leave archived unless ident support is intentionally restored.
...
3. **`src/act_wiz.c` line 232-234**: Three `fgets()` calls reading 80-byte lines:
   ...
   - **Status**: Fixed by replacing the literal with `sizeof(arg)`.

### String safety infrastructure
- **Implementation**: `src/string_safe.c` provides OpenBSD-style `toc_strlcpy()` and `toc_strlcat()` helpers with guaranteed null-termination and overflow-detectable return values.
- **Prototypes**: Declared in `src/merc.h`.
- **Local helpers**: `src/act_comm.c` and `src/comm.c` define static `safe_strcpy`/`safe_strcat` wrappers that call the global `toc_strlcpy`/`toc_strlcat` helpers.
- **Coverage**: Most formatted output now uses `snprintf()` with `sizeof()` limits; concatenation uses bounded helpers with proper buffer sizes.
...
## Python Web Admin Review (Nov 2025; status updated July 2026)
...
2. **Authentication/authorization**: Operational endpoints were originally public.
   **Impact**: Anyone who could reach the web admin port could shut down the server, run arbitrary immortal commands, or trigger backups.
   **Status**: Fixed for operational endpoints through `WEB_ADMIN_TOKEN` and the `X-Admin-Token` header. If no server token is configured, those endpoints are disabled with HTTP 503. General browsing endpoints such as health, stats, area health, and parsed area/object/room views remain public by design. Public deployments should still use firewall rules, VPN, or a TLS reverse proxy.
...
3. **Command injection risk** (theoretical): The `/api/command` endpoint queues arbitrary text that the game loop later executes through `interpret()`. While this is by design (admin commands), it means any compromise of the web admin grants full immortal command access.
   **Mitigation**: Keep `WEB_ADMIN_TOKEN` set, restrict network access to trusted operators, and consider an allowlist if full command flexibility is not required.

4. **Log file reading**: `/api/logs` originally read the entire log file into memory.
   **Impact**: Low with normal log rotation, but large logs can still matter.
   **Status**: Improved. POSIX hosts use `tail`, requested line counts are clamped, and log access requires a configured, matching `WEB_ADMIN_TOKEN`. Windows still uses a read-and-slice fallback.
...
- HTML dashboard is functional and self-contained (no external frontend build).
- Area-health logic is shared between the dashboard, API, CLI, and unit tests.

## Legacy Code & Dead Files (Nov 2025)
1. **`archive/resolve.c`**: Standalone ident protocol resolver (RFC 1413) with unsafe string operations. Previously lived under `area/`; now archived and not compiled or linked. Appears to be legacy code from when the MUD performed reverse DNS and ident lookups on connecting users.
   - **Status**: Archived historical reference, not active runtime code.
   - **Action**: Leave archived unless explicitly reviving ident support.
...
1. **macOS build compatibility**: Two historical issues were tracked:
   - Missing `<crypt.h>` header on macOS.
   - Name collision with system `strlcpy`/`strlcat`.
   - **Status**: Improved. `merc.h` skips `<crypt.h>` on Apple platforms, and the local bounded string helpers now use the `toc_strlcpy`/`toc_strlcat` names instead of overriding platform functions. Docker remains the recommended deployment build for consistency.
...
4. **Web admin authentication**: Operational endpoints require `WEB_ADMIN_TOKEN`.
   - **Status**: FIXED for logs, live log WebSocket, backup listing, reload, command, wizinfo, backup, and shutdown. They remain disabled until a token is configured.
   - **Remaining risk**: Port 9001 is still plain HTTP, and parsed browsing endpoints remain public by design. Use firewall rules, VPN, or a TLS reverse proxy for public deployments.
...
### Low (cleanup)
6. **Legacy unsafe code in `archive/resolve.c`**: Three unsafe string calls remain in archived dead code.
   - **Status**: Archived outside the active area directory; not compiled or linked.
   - **Action**: Convert only if ident support is intentionally restored.

7. **Documentation files in `src/`**: `swedish.txt`, `swe.txt`, `points` clutter source directory.
   - **Status**: Still present in `src/`; not compiled.
   - **Action**: Move or delete in a dedicated cleanup pass if desired.
...
- **Primary workflow**: Use Docker for deployment and the native Makefile inside Linux/WSL for fast development iteration.
- **Local testing**: Run `.\scripts\validate.ps1` on Windows or `bash scripts/validate.sh` on Linux/macOS. Use `-RunSmoke` or `RUN_SMOKE=1` before deployment-facing changes.
- **CI/CD**: GitHub Actions now runs `.github/workflows/validate.yml`, which installs Python dependencies and executes `bash scripts/validate.sh`.
- **Deep memory testing**: CMake sanitizer builds and Valgrind remain useful follow-up checks for gameplay sessions, especially after changes in combat, update loops, saving/loading, or command interpretation.

### Security Posture
- **Current state**: String buffer overflows largely mitigated via bounded string functions
- **Web admin**: `WEB_ADMIN_TOKEN` is required for operational endpoints; they return HTTP 503 while it is unset. Port 9001 is still plain HTTP, so public deployments should use firewall rules, a VPN, or a TLS reverse proxy.
- **Player data**: Not reviewed per policy; assumed to be handled correctly
- **Area files**: Trusted input (admin-created), but now covered by parser checks, `merc --check-area`, reference validators, and area-health linting for structural regressions.
...
## Security Posture (April 2026; status updated July 2026)

- ✅ `WEB_ADMIN_TOKEN` auth — operational endpoints require a configured token and matching `X-Admin-Token` header
...
- **Improved**: Area files remain trusted admin input, but `merc --check-area`, legacy reference checkers, and area-health linting now catch structural regressions before deployment
