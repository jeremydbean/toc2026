...
# Configuration
MUD_HOST = os.getenv("MUD_HOST", "mud-server")
MUD_PORT = int(os.getenv("MUD_PORT", 4000))
WS_PORT = int(os.getenv("WS_PORT", 8081))


async def forward_mud_to_ws(reader, websocket):
...
async def forward_ws_to_mud(writer, websocket):
    ...
    try:
        ...
        print(f"Error sending to MUD: {e}")


async def mud_handler(websocket, path):
...
def main():
    ...
    start_server = websockets.serve(mud_handler, "0.0.0.0", WS_PORT)
