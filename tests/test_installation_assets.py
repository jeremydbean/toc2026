...
class InstallationAssetsTests(unittest.TestCase):
    ...
    def test_pi_updater_is_periodic_and_portal_triggered(self):
        ...
        self.assertIn("tests.test_mail_notifier", updater)
    ...
    def test_pi_email_notifications_are_secret_backed_bounded_and_event_driven(self):
        notifier = read("deploy/toc2026_mail.py")
        example = read("deploy/mail.env.example")
        service = read("deploy/systemd/toc2026-mail@.service")
        network_timer = read("deploy/systemd/toc2026-network-notify.timer")
        installer = read("deploy/install-pi.sh")
        game_service = read("deploy/systemd/toc2026-game.service")
        web_service = read("deploy/systemd/toc2026-web.service")
        healthcheck = read("deploy/toc2026-healthcheck")
        stable = read("deploy/toc2026-stable")

        self.assertIn("TOC_SMTP_PASSWORD=replace-with-smtp-password", example)
        self.assertIn("stat.S_IMODE", notifier)
        self.assertIn("sanitize_log", notifier)
        self.assertIn("MAX_JOURNAL_OUTPUT", notifier)
        self.assertIn("Passwords and private keys are never included", notifier)
        self.assertIn("ConditionPathExists=/etc/toc2026/mail.env", service)
        self.assertIn("MemoryMax=64M", service)
        self.assertIn("ReadWritePaths=/var/lib/toc2026/mail", service)
        self.assertIn("OnUnitActiveSec=5min", network_timer)
        self.assertIn("toc2026-network-notify.timer", installer)
        self.assertIn("toc2026-mail@startup.service", installer)
        self.assertIn("toc2026-mail@game-failure.service", game_service)
        self.assertIn("toc2026-mail@web-failure.service", web_service)
        self.assertIn("toc2026-mail@healthcheck-repair.service", healthcheck)
        self.assertIn("toc2026-mail@recovered.service", stable)
