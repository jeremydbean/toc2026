...
bool spec_psionic( CHAR_DATA *mob, CHAR_DATA *ch, DO_FUN *cmd, char *arg )
{
...

    {
        ...
        psi_sel = number_range( 1, psi_max );
        ...
        case  5: do_pyrotechnics(mob, victim->name); break;
        case  6: do_confuse( mob, victim->name);  break;
        case  7: do_enervate( mob, victim->name); break;
        case  8: do_mindleech( mob, victim->name); break;
        case  9: do_mindblast( mob, victim->name);break;
...
bool spec_executioner( CHAR_DATA *mob, CHAR_DATA *ch, DO_FUN *cmd, char *arg )
{
	...
	if ( IS_SET(victim->comm,COMM_WHINE) )
	{
		...
		case 13:
                    snprintf(buf, sizeof(buf),"%s, don't make me turn this carriage around and go home?",
                        victim->name);
		    break;
