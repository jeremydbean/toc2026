...
void trapped( CHAR_DATA *ch, OBJ_DATA *obj, int find_trap )
{
  ...
  if(IS_NPC(ch) )
   ...
   act("$n set off a trap!",ch,NULL,NULL,TO_ROOM);
    ...
    snprintf(buf, sizeof(buf), "%s has set off a Fire trap!!!", ch->name);
	 ...
	 for ( gch = ch->in_room->people; gch; gch = gch->next_in_room )
	 {
	  if(!IS_NPC(gch) )
	  {
	   ...
	   if(type_gas == 2)
	   {
              af.type = (int16_t)skill_lookup("sleep");
	     af.level     = gch->level/2;
	     af.duration  = 12;
	     af.location  = APPLY_NONE;
	     af.modifier  = 0;
	...
	ch->mana = 10;
	ch->move = 10;
         af.type      = (int16_t)skill_lookup("curse");
	af.level     = ch->level/2;
         af.duration  = (int16_t)dice(1,2);
    ...
    snprintf(buf, sizeof(buf), "%s has set off a Stun trap!!!", ch->name);
	 ...
	 send_to_char("Something has been injected into your body!\n\r",ch);
         af.type      = (int16_t)skill_lookup("plague");
	 af.level     = ch->level/2;
	 af.duration  = 12;
    ...
    snprintf(buf, sizeof(buf), "%s has set off a Plague trap!!!", ch->name);
	 ...
	 if(number_percent ( ) > 50)
	 {
	   for ( gch = ch->in_room->people; gch; gch = gch->next_in_room )
	   {
	    if(!IS_NPC(gch) )
	    {
              af.type      = (int16_t)skill_lookup("blindness");
              af.level     = (int16_t)(gch->level/3);
	      af.location  = APPLY_HITROLL;
	      af.modifier  = -4;
